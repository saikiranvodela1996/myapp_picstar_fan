...
public class MyImageView extends AppCompatImageView {
    ...
    public Bitmap getUnCropedBitmap(){
        Bitmap source = getBitmap();
        if (source == null) return null;
        if (mCropMode != MyImageView.CropMode.CIRCLE) return source;
        return getCircularBitmap(source);
    }
    // Handle styleable ////////////////////////////////////////////////////////////////////////////

    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        TypedArray ta = context.obtainStyledAttributes(attrs, com.isseiaoki.simplecropview.R.styleable.scv_CropImageView, defStyle, 0);
        Drawable drawable;
        ...
        try {
            ...
            mBackgroundColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_background_color, TRANSPARENT);
            super.setBackgroundColor(mBackgroundColor);
            mOverlayColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_overlay_color, TRANSLUCENT_BLACK);
            ...
            mHandleSize = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_size, (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_touch_padding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_min_frame_size, (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_frame_stroke_weight, (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_stroke_weight, (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mIsCropEnabled = ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_crop_enabled, true);
            mInitialFrameScale = constrain(ta.getFloat(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_initial_frame_scale, DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
